 ...
 * @nosideffects
 */
export function getNeighborCoords(coord: Coord, iMax: number, jMax: number) {
 ...
 * @nosideffects
 */
function getNeighborCoordsInterior(coord: Coord): Array<Coord> {
  const i = coord[0];
  const j = coord[1];
  return [
 ...
 * @nosideffects
 */
function filterNeighborCoords(neighborCoords: Array<Coord>, iMax: number, jMax: number): Array<Coord> {
  let filteredCoords: Array<Coord> = [];
 ...
 * @nosideffects
 */
function coordInBounds(coord: Coord, iMax: number, jMax: number): boolean {
  return (
    (0 <= coord[0] && coord[0] <= iMax)
 ...
 * @nosideeffects
 */
function coordOnEdge(coord: Coord, iMax: number, jMax: number): boolean {
  return (
    coord[0] === 0
    || coord[0] === iMax
    || coord[1] === 0
