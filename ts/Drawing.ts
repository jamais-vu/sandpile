...
export class Drawing {
  ...
  private cellColors: Map<number, string>;
   ...
   */
  constructor(ctx: CanvasRenderingContext2D, cellSize: number, cellColors: Map<number, string>) {
    this.ctx = ctx;
   ...
   * @modifies {this.ctx}
   */
  drawGrid(option: 'full' | 'next', rows: number, cols: number, currentGrid: Grid, previousGrid?: Grid): void {
    for (let i = 0; i < rows; i++) {   // i is y-axis (row index).
      for (let j = 0; j < cols; j++) { // j is x-axis (column index).
        const cellState: number = currentGrid[i][j];
   ...
   * @modifies {this.ctx}
   */
  drawCell(i: number, j: number, cellState: number): void {
    /* Arguments for clearRect() and fillRect() */
    const rectArgs: [number, number, number, number] = [
      (j * this.cellSize), // x-coord of upper-left of cell
      (i * this.cellSize), // y-coord of upper-left of cell
      this.cellWidth,      // width of cell square
    ...
    if (cellState === 0) {
      ...
      this.ctx.fillStyle = this.cellColors.get(cellState) as string;
   ...
   * @modifies {this.ctx}
   */
  drawGridLines(color: string, rows: number, cols: number): void {
