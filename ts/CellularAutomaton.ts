...
export class CellularAutomaton {
   ...
   */
  constructor(grid: Grid, transitionFunction: TransitionFunction<Grid, any>) {
    this.grid = grid;
  ...
  previousStep(): void {
    if (this.step > 0) {
      ...
      this.grid = this.history.pop() as Grid;
      this.step -= 1;
    }
  }
  ...
  goToStep(n: number): void {
    if (n > this.step) {
      ...
      const stepsToGo: number = (n - this.step);
      for (let i = 1; i <= stepsToGo; i++) {
        this.nextStep();
      }
    } else if (n < this.step) {
      // Step is in the past.
      const stepsToGo: number = (this.step - n);
      for (let i = 1; i <= stepsToGo; i++) {
        this.previousStep();
      }
    }
   ...
   * @nosideeffects
   */
  getPreviousGrid(): Grid | undefined {
    if (this.step > 0) {
   ...
   * @modifies {this.grid}
   */
  setCellState(i: number, j: number, state: number): void {
    this.grid[i][j] = state;
  }
   ...
   * @nosideeffects
   */
  getPreviousCellState(i: number, j: number): number {
    if (this.step === 0) {
      return NaN;
    } else {
